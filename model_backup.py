...
# return_list = (last_price, brokerage_fee, current_balance, trade_volume, agg_balance, username, ticker_symbol, current_number_shares)
    ...
    cursor.execute("""
        ...
        '{}',{},'{}',{}
        );""".format(ticker_symbol,trade_volume*-1,username[0],last_price)
    )
    #holdings
...
def buy_db(return_list): # return_list = (last_price, brokerage_fee, current_balance, trade_volume, left_over, username, ticker_symbol)
    ...
    cursor.execute( """
        ...
        '{}',{},'{}',{}
        );""".format(ticker_symbol,trade_volume,username[0],last_price)
    )
...
def get_user_balance(username):
    ...
    username = current_user()
    print(username)
    query = 'SELECT current_balance FROM user WHERE username = "{}";'.format(username[0])
#    print(f'query{query}')
    cursor.execute(query)
